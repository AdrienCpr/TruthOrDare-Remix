    ...
    >
      ...
      >
        ...
        <div className="relative z-10 flex items-center justify-center">
          <div className="relative h-44 w-44 srounded-full flex items-center justify-center text-white font-bold text-2xl">
            <div className="absolute w-full h-full blur-sm rounded-full" />
            <span className="text:white z-20 text-4xl">{text}</span>
          </div>
        </div>
      </div>
    </div>
...
export function CardPattern({ mouseX, mouseY, randomString }: any) {
  ...
  return (
    <div className="pointer-events-none">
      ...
      <motion.div
        className="absolute inset-0 rounded bg-gradient-to-r from-gray-700 to-gray-900 opacity-0  group-hover/card:opacity-100 backdrop-blur-xl transition duration-500"
        style={style}
      />
      <motion.div
